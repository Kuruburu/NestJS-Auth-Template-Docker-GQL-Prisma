...
const mockPrismaService = {
  ...
  refreshToken: {
    ...
    create: jest.fn().mockResolvedValue('elo'),
  },
  $transaction: jest.fn(),
};
  ...
  getOrThrow: jest.fn().mockImplementation((key: string) => {
    if (key === 'security') {
      return {
        ...
        refreshTokenShortExpirationInHours: 1,
      };
    }
    return undefined;
  }),
...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: PasswordService, useValue: { hashPassword: jest.fn(), validatePassword: jest.fn() } },
  ...
  describe('login', () => {
    it('should return tokens if login is successful', async () => {
      ...
      const result = await authService.login(mockUser, false);
  ...
  describe('validateUser', () => {
    it('should return user if credentials match', async () => {
      jest.spyOn(userService, 'findOneByEmailWithPassword').mockResolvedValue(mockUser);
      ...
      const userDto = plainToInstance(UserDto, mockUser);
      expect(result).toBeInstanceOf(UserDto);
      expect(result).toEqual(userDto);
      expect(userService.findOneByEmailWithPassword).toHaveBeenCalledWith(mockUser.email);
      expect(passwordService.validatePassword).toHaveBeenCalledWith('password', mockUser.passwordHash);
    });

    it('should return null if email does not exist', async () => {
      jest.spyOn(userService, 'findOneByEmailWithPassword').mockResolvedValue(null);
      await expect(authService.validateUser(mockUser.email, 'password')).resolves.toBeNull();
    });
